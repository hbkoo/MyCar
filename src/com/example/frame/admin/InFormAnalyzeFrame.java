...
public class InFormAnalyzeFrame extends JFrame implements ActionListener {
	 ...
	 * Create the frame.
	 */
	public InFormAnalyzeFrame(Staff staff, JFrame parent) {
		this.staff = staff;
		this.parent = parent;
		...
		JLabel label = new JLabel("进车单统计", SwingConstants.CENTER);
	...
	private void updateRepAndModelBox() {
	...
	private void DoSearch() {
		...
		try {
			InFormsDAO inFormsDAO = InFormsDAO.getInstance();
			List<InForm> inForms = inFormsDAO.getInForms(salesman_id, rep_id, model, start_time, end_time, tag);
			...
			table.updateUI();
			result_label.setText(String.valueOf(inForms.size()));
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			JOptionPane.showMessageDialog(parent, "数据库连接错误", "错误", JOptionPane.ERROR_MESSAGE);
